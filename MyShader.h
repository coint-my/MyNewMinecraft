...
{
private:
	std::unordered_map<std::string, GLint> uniformCacheMatrix4x4;
public:
	unsigned int ID;
	...
	~MyShader()
	{
		uniformCacheMatrix4x4.clear();
		std::cout << "delete shader ID = " << ID << std::endl;
		glDeleteShader(ID);
	}
	MyShader() {}
	MyShader(const char* vertexPath)
	{
	...
	{
		auto it = uniformCacheMatrix4x4.find(name);
		if (it != uniformCacheMatrix4x4.end())
			return it->second;

		GLint loc = glGetUniformLocation(ID, name.c_str());
		uniformCacheMatrix4x4[name] = loc;
		return loc;
	}
	// ------------------------------------------------------------------------
